...
object MockVideoFactory{
    fun getPlayList(videoCount:Int):PlayList{
        var list = mutableListOf<Video>()
        for(i in 0 until videoCount){
            var categoryId = (i+1).toString()
            list.add(getVideo(i+1))
        }
        return PlayList(items =  list)
    }

    fun getVideo(id:Int = 1,content:String = "dummy-content",dummyUrl:String = "http:dummy.com"):Video{
